...
public:
  any &Get(CLSLocation loc) { return map_[loc]; }
};
...
T &GetSpecific(CLSLocation loc, Args &&... args) {
  ...
  any &val = m->Get(loc);
  if (val.empty()) {
    //        std::cout << "Set<T> val:" << (void*)&val << ", m:" << (void*)m <<
    //        std::endl; std::cout << "sizeof:" << sizeof...(args) << std::endl;
...
template <typename T> class CLSRef {
  CLSLocation loc_;
...
public:
  ...
  operator T const &() const { return GetSpecific<T>(loc_); }

  operator T &() { return GetSpecific<T>(loc_); }
};
...
#define CLS(type, ...) co::MakeCLSRef<type>(GetCLSLocation(), ##__VA_ARGS__)
