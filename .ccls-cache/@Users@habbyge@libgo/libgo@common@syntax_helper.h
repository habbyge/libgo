...
template <int OptType> struct __go_option;

template <> struct __go_option<opt_scheduler> {
  Scheduler *scheduler_;
  explicit __go_option(Scheduler *scheduler) : scheduler_(scheduler) {}
  explicit __go_option(Scheduler &scheduler) : scheduler_(&scheduler) {}
};

template <> struct __go_option<opt_stack_size> {
  std::size_t stack_size_;
  explicit __go_option(std::size_t v) : stack_size_(v) {}
};

template <> struct __go_option<opt_affinity> {
  bool affinity_;
...
struct __go {
  ...
  template <typename Function> ALWAYS_INLINE void operator-(Function const &f) {
    if (!scheduler_)
      scheduler_ = Processer::GetCurrentScheduler();
    if (!scheduler_)
