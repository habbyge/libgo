...
void Scheduler::CreateTask(TaskF const& fn, TaskOpt const& opt) {
    Task* tk = new Task(fn, opt.stack_size_ ? opt.stack_size_ 
...
//    printf("new tk = %p  impl = %p\n", tk, tk->impl_);
    ...
    DebugPrint(dbg_task, "task(%s) created in scheduler(%p).", 
            TaskDebugInfo(tk), (void*)this);
#if ENABLE_DEBUGGER
...
void Scheduler::UseAloneTimerThread() {
    ...
    if (!started_.try_lock()) {
        ...
        std::thread t([this, timer] { 
            DebugPrint(dbg_thread, "Start alone timer(sched=%p) thread id: %lu", 
