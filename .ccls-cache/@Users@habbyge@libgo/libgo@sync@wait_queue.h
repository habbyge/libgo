...
public:
  ...
  explicit WaitQueue(Functor checkFunctor = NULL, size_t posD = -1,
                     Functor const &posFunctor = NULL)
      : posDistance_(posD) {
    head_ = &dummy_;
    tail_ = &dummy_;
    check_ = &dummy_;
  ...
  CondRet push(T *ptr, std::function<CondRet(size_t)> const &cond = NULL) {
    ...
    // check
    if (count_ < (std::max)(posDistance_, posDistance_ + 16))
      return ret;
    if (!checkFunctor_)
      return ret;
