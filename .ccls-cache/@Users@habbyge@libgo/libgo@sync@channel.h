...
public:
  ...
  // @choose2: if capacity less than choose2, use ringbuffer. else use
  // std::list.
  ...
  void SetDbgMask(uint64_t mask) { impl_->SetDbgMask(mask); }
  ...
  bool TryPush(T t) const { return impl_->Push(t, false); }

  bool TryPop(T &t) const { return impl_->Pop(t, false); }
  ...
  template <typename Rep, typename Period>
  bool TimedPop(std::nullptr_t ignore,
  ...
  bool TimedPop(std::nullptr_t ignore,
                FastSteadyClock::time_point deadline) const {
    T t;
    return impl_->Pop(t, true, deadline);
  }
...
template <> class Channel<void> : public Channel<std::nullptr_t> {
public:
  explicit Channel(std::size_t capacity = 0)
      : Channel<std::nullptr_t>(capacity) {}
};

template <typename T> using co_chan = Channel<T>;
