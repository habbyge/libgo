...
#define DebugPrint(type, fmt, ...)                                             \
  do {                                                                         \
    if (UNLIKELY(::co::CoroutineOptions::getInstance().debug & (type))) {      \
      ...
      fprintf(::co::CoroutineOptions::getInstance().debug_output,              \
              "[%s][%05d][%04d][%06d]%s:%d:(%s)\t " fmt "\n",                  \
              ::co::GetCurrentTimeStr().c_str(), ::co::GetCurrentProcessID(),  \
              ::co::GetCurrentThreadID(), ::co::GetCurrentCoroID(),            \
              ::co::BaseFile(__FILE__), __LINE__, __FUNCTION__,                \
              ##__VA_ARGS__);                                                  \
